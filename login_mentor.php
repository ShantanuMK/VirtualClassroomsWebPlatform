...
if(isset($_POST['login'])){
 ...
 if($rows == 1){
    $ssiid=$datab["school_id"];
    $app=mysqli_query($conn,"SELECT * FROM schools where school_id=$ssiid ");
    $afet=mysqli_fetch_assoc($app);
    if ($afet["approve"]==2) {
       $_SESSION['tid']=$datab["tid"];
       $_SESSION['tname']=$datab["tname"];
       $_SESSION['school_id']=$datab['school_id'];
       $_SESSION['temail']=$datab['temail'];
       header("Location: dash_mentor.php");
      }
      elseif ($afet["approve"]==1) {
        echo '<script>alert("Your Services are stopped temporarily.")</script>';
      }
      elseif ($afet["approve"]==0) {
        echo '<script>alert("Your Services are not yet activated.")</script>';
      }
